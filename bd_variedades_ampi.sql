...
SELECT dp.id_pedido as pedido, p.fecha_pedido as fecha,p.fecha_entrega as entrega,pr.nombre_producto as producto
,dp.cantidad,dp.precio_unida as precio,dp.destino_pedido as direccion,p.estado_pedido as estado,dp.precio_unidad*dp.cantidad 
AS costeporproducto FROM `detalles_pedido` dp INNER JOIN pedido p 
...
---------- scrip para la factura 
vistas
CREATE VIEW facturas as
SELECT dp.id_pedido as pedido, pr.nombre_producto as producto,dp.cantidad, dp.precio_unidad as precio 
,dp.precio_unidad*dp.cantidad AS costeporproducto FROM `detalles_pedido` dp INNER JOIN pedido p 
ON dp.id_pedido=p.id_pedido INNER join producto pr on dp.id_producto=pr.id_producto;

procedimientos

CREATE PROCEDURE `factura`(IN `codigo` INT) 
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY DEFINER BEGIN SELECT * ,totaldepedido(codigo)as total ,
descuentos(totaldepedido(codigo),SUM(cantidad)) as totalcondescuento FROM facturas WHERE pedido = codigo GROUP BY producto; END
